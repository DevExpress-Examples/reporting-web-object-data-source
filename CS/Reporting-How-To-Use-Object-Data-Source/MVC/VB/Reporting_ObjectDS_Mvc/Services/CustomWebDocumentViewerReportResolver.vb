			...
			Else
				...
				Else
					If reportName.EndsWith("Parameter") Then
						...
						Dim parameter = New Parameter() With {.Name = "noOfItems", .Type = GetType(DevExpress.DataAccess.Expression), .Value = New DevExpress.DataAccess.Expression("[Parameters." & "parameterNoOfItems" & "]", GetType(Integer))}
						dataSource.Constructor = New ObjectConstructorInfo(parameter)
						dataSource.DataMember = "Items"
						Return dataSource
						'				#End Region
					Else
						...
						dataSource.Name = "EmployeeObjectDS"
						dataSource.DataSource = GetType(Employees.EmployeeList)
						Dim parameterNoOfItems = New Parameter("noOfItems", GetType(Integer), 0)
